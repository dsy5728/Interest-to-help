...
CREATE TABLE `t_news_type` (
	`id` varchar(32) NOT NULL COMMENT '主键ID',
	`name` varchar(32) DEFAULT NULL COMMENT '新闻类型名称',
	`path` varchar(500) DEFAULT NULL COMMENT '进入该类型的URL',
	`ids` varchar(200) DEFAULT NULL COMMENT '爬取的IDS(来自网络)',
	`parent_id` varchar(32) DEFAULT NULL COMMENT '上级类型code(来自网络)',
  PRIMARY KEY (`id`) 
) ENGINE=InnoDB AUTO_INCREMENT=47 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='新闻导航类型表';

CREATE TABLE `t_news_content` (
	`id` varchar(32) NOT NULL COMMENT '主键ID',
	`content` text DEFAULT NULL COMMENT '新闻内容',
	`content_css` text DEFAULT NULL COMMENT '内容样式',
	`type_id` varchar(32) DEFAULT NULL COMMENT '新闻类型ID  t_news_type PK(id)',
	`news_module` varchar(150) DEFAULT NULL COMMENT '新闻所属版块  今日话题:today  热点精选:hot 今日要闻:top',
	`title` varchar(100) DEFAULT NULL COMMENT '新闻标题',
	`article_id` varchar(100) DEFAULT NULL COMMENT '文章id(来自网络定义)',
	`comment_id` varchar(32) DEFAULT NULL COMMENT '未知ID(来自网络定义)',
	`url` varchar(500) DEFAULT NULL COMMENT '文章内容详情页url(来自网络定义)',
	`group1` varchar(32) DEFAULT NULL COMMENT '分组code(来自网络定义)',
	`publish_time` varchar(32) DEFAULT NULL COMMENT '发布时间',
	`source` varchar(32) DEFAULT NULL COMMENT '来源 例如: 中国新闻网、海外网',
  PRIMARY KEY (`id`) 
) ENGINE=InnoDB AUTO_INCREMENT=47 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='新闻详情内容';
	
CREATE TABLE `t_news_imgfile` (
	`id` varchar(32) NOT NULL COMMENT '主键ID',
	`content_id` varchar(32) DEFAULT NULL COMMENT '新闻ID  t_news_content PK(id)',
	`news_module` varchar(32) DEFAULT NULL COMMENT '新闻所属版块',
	`img_source` varchar(500) DEFAULT NULL COMMENT '图片源路径（网络URL）',
	`img_local` varchar(500) DEFAULT NULL COMMENT '图片本地路径',
	`img_position` varchar(32) DEFAULT NULL COMMENT '图片位置类型， 列表图片:0，  详情内容图片:1',
  PRIMARY KEY (`id`) 
) ENGINE=InnoDB AUTO_INCREMENT=47 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='新闻对应的图片';

 CREATE TABLE `t_news_favorites` (
	`id` varchar(32) NOT NULL COMMENT '主键ID',
	`content_id` varchar(32) DEFAULT NULL COMMENT '新闻ID  t_news_content PK(id)',
	`user_id` varchar(32) DEFAULT NULL COMMENT '新闻ID  sys_user PK(id)',
	`create_time` datetime default NULL COMMENT '创建时间',
  PRIMARY KEY (`id`) 
) ENGINE=InnoDB AUTO_INCREMENT=47 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT COMMENT='新闻收藏列表';

alter table t_news_content add type_name varchar(50) default null COMMENT '类型名称';
alter table t_news_content add header_img varchar(250) default null COMMENT '列表图片URL';
alter table t_news_content add bimg varchar(250) default null COMMENT '列表图片URL';

-- 2019.04.24 21:39 UPDATE
alter table sys_user add interest varchar(300) default null COMMENT '用户兴趣标签';